...
public class EstrategiaPreco implements AnaliseEstrategia {
    ...
    public void analisar(
        ...
        for (Estrategia estrategia : estrategias) {
            ...
            boolean isPrecoSubiu =
                    ((precoAtual.compareTo(precoAlvo) > 0)
                            && (precoAnterior.compareTo(precoAlvo) < 0));
            boolean isPrecoDesceu =
                    ((precoAtual.compareTo(precoAlvo) < 0)
                            && (precoAnterior.compareTo(precoAlvo) > 0));
            ...
            if (isPrecoSubiu || isPrecoDesceu) {
                ...
                log.info(
                        "Alarme acionado para o símbolo: {} com preço alvo: {}",
                        simboloIntervalo,
                        precoAlvo);
            }
        }
