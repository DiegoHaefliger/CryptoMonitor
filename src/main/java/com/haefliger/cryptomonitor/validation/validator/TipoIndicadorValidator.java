...
public class TipoIndicadorValidator
        implements ConstraintValidator<TipoIndicadorValido, TipoIndicadorEnum> {
    @Override
    public boolean isValid(TipoIndicadorEnum value, ConstraintValidatorContext context) {
        if (value == null || !TipoIndicadorEnum.isValid(value.name())) {
            ...
            context.buildConstraintViolationWithTemplate(
                            "Tipo de indicador inválido. Valores válidos: "
                                    + TipoIndicadorEnum.valoresValidos())
