...
public class OperadorLogicoValidator
        implements ConstraintValidator<OperadorLogicoValido, OperadorLogicoEnum> {
    @Override
    public boolean isValid(OperadorLogicoEnum value, ConstraintValidatorContext context) {
        if (value == null || !OperadorLogicoEnum.isValid(value.name())) {
            ...
            context.buildConstraintViolationWithTemplate(
                            "Operador lógico inválido. Valores válidos: "
                                    + OperadorLogicoEnum.valoresValidos())
