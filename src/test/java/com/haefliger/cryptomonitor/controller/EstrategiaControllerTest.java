...
class EstrategiaControllerTest {
    ...
    @InjectMock EstrategiaService estrategiaService;
    ...
    void salvarEstrategia() {
        when(estrategiaService.salvarEstrategia(any()))
    ...
    void buscarEstrategia() {
        BuscarEstrategiaListaResponse item =
                new BuscarEstrategiaListaResponse(
                        1L,
                        "Bitcoin RSI",
                        "BTCUSDT",
                        "60",
    ...
    void buscarEstrategiaRepassaFiltroAtivo() {
        ...
        given().queryParam("ativo", true).when().get("/estrategia").then().statusCode(200);
    ...
    void deletarDevolve204SemCorpo() {
        given().queryParam("id", 5).when().delete("/estrategia").then().statusCode(204);
    ...
    void statusDevolve204SemCorpo() {
        given().queryParam("id", 5)
                ...
                .queryParam("permanente", true)
                .when()
                .put("/estrategia/status")
                .then()
                .statusCode(204);
    ...
    void payloadInvalidoDevolve400() {
        EstrategiaRequest invalida =
                new EstrategiaRequest(
                        "",
                        "BTCUSDT",
                        "7",
                        OperadorLogicoEnum.AND,
                        Boolean.FALSE,
                        requestValida().condicoes());
    ...
    @Test
    @DisplayName(
            "DELETE sem id devolve 400, não 500 — @QueryParam sozinho seria opcional, diferente do @RequestParam do Spring")
