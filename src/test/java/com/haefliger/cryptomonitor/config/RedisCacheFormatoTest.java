...
@QuarkusTest
@DisplayName("Formato do cache Redis — contrato congelado na F0, agora produzido pelo ObjectMapper do Quarkus")
class RedisCacheFormatoTest {
    ...
    private static List<EstrategiaCacheDTO> amostra() {
        CondicaoEstrategiaCacheDTO condicao = new CondicaoEstrategiaCacheDTO(
                10L, "RSI", OperadorComparacaoEnum.MENOR, "30",
                LocalDateTime.parse("2026-01-01T10:00:00"));
        return List.of(new EstrategiaCacheDTO(
                1L, "Bitcoin RSI", "BTCUSDT", "60", "AND",
                LocalDateTime.parse("2026-01-01T10:00:00"), null,
                Boolean.TRUE, Boolean.FALSE, List.of(condicao)));
    }

    @Test
    @DisplayName("o JSON gravado é byte a byte o mesmo da versão Spring: data ISO-8601, nulo explícito, enum pelo name()")
    void formatoGravado() throws Exception {
        assertThat(objectMapper.writeValueAsString(amostra())).isEqualTo(JSON_CONGELADO);
    }

    @Test
    @DisplayName("o cache escrito pela versão Spring continua legível — sem \"@class\", direto para o record")
    void leEscritoPelaVersaoAnterior() throws Exception {
        ...
        List<EstrategiaCacheDTO> lido =
                objectMapper.readValue(JSON_CONGELADO, new TypeReference<List<EstrategiaCacheDTO>>() {
                });

        assertThat(lido).singleElement().satisfies(dto -> {
            assertThat(dto.id()).isEqualTo(1L);
            assertThat(dto.ativo()).isTrue();
            assertThat(dto.dateCreated()).isEqualTo(LocalDateTime.parse("2026-01-01T10:00:00"));
            assertThat(dto.condicoes()).singleElement()
